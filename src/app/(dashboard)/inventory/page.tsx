...
function InventoryContent() {
  ...
  const fetchData = useCallback((force = false) => {
    ...
    params.set("includeMeta", page === 1 ? "true" : "false");
    if (userRole !== "ADMIN") {
    ...
    fetch(`/api/inventory?${params}`, { signal: controller.signal })
      .then(async (response) => {
        ...
        if (!response.ok) throw new Error(payload.error || "Không thể tải kho phụ tùng");
        return payload;
      })
      .then((payload) => {
        setData((previous: any) => ({
          ...previous,
          ...payload,
          categories: payload.categories ?? previous?.categories ?? [],
          vehicleModels: payload.vehicleModels ?? previous?.vehicleModels ?? [],
  ...
  const lowStock = rawProducts.filter((p: any) => Number(p.stockCount) <= Number(p.stockMin));
  const highStock = rawProducts.filter((p: any) => Number(p.stockCount) >= Number(p.stockMax));
  const pageTotalValue = rawProducts.reduce((sum: number, p: any) => {
    const retail = (p.prices || []).find((pr: any) => pr.type === "RETAIL");
    return sum + (retail ? Number(retail.amount) * Number(p.stockCount) : 0);
  }, 0);
  const pageTotalInsuranceValue = rawProducts.reduce((sum: number, p: any) => {
    const insurance = (p.prices || []).find((pr: any) => pr.type === "INSURANCE");
    return sum + (insurance ? Number(insurance.amount) * Number(p.stockCount) : 0);
  }, 0);
      ...
      <div className="space-y-6 stagger">
        ...
        {user?.role !== "ADMIN" && (
          <div className="flex border-b border-border">
            <button
              ...
              className={`px-6 py-3 text-sm font-semibold transition-all border-b-2 -mb-[2px] ${scope === "current"
            ...
            <button
              ...
              className={`px-6 py-3 text-sm font-semibold transition-all border-b-2 -mb-[2px] ${scope === "other"
                  ? "border-primary text-primary font-bold"
                  : "border-transparent text-muted-foreground hover:text-foreground"
                }`}
            >
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <div
            ...
            className={`rounded-xl p-4 transition-all duration-200 cursor-pointer ${statFilter === "all"
          ...
          {/* Card 3: Sắp hết hàng */}
          <div
            onClick={() => { setStatFilter("low"); setPage(1); }}
            className={`rounded-xl p-4 transition-all duration-200 cursor-pointer ${statFilter === "low"
          ...
          {/* Card 4: Tồn cao */}
          <div
            onClick={() => { setStatFilter("high"); setPage(1); }}
            className={`rounded-xl p-4 transition-all duration-200 cursor-pointer ${statFilter === "high"
              ...
              {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-1 rounded-lg text-xs font-semibold border ${p === page
      ...
      {modalOpen && (
        <ModalPortal>
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-md animate-fade-in">
            <div className="w-full max-w-2xl bg-card border border-border/80 rounded-3xl overflow-hidden shadow-2xl animate-slide-in-bottom">
              {/* Modal Header */}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-base font-bold text-foreground">
                      {editingId ? "Cập nhật thông tin phụ tùng" : "Thêm phụ tùng mới vào kho"}
                    </h3>
                    <p className="text-xs text-muted-foreground">
                      {editingId ? "Chỉnh sửa mã SKU, định mức tồn kho và các mức giá" : "Điền đầy đủ thông tin chi tiết để quản lý tồn kho"}
                    </p>
                  </div>
                </div>
                <button
                  ...
                  aria-label="Đóng biểu mẫu phụ tùng"
                >
                  <X size={18} />
                </button>
              </div>
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[82vh] overflow-y-auto">
                {formMessage && (
                  <div role="alert" className={`rounded-xl border px-3.5 py-2.5 text-xs font-semibold ${formMessage.type === "error" ? "border-destructive/30 bg-destructive/10 text-destructive" : "border-emerald-500/30 bg-emerald-500/10 text-emerald-600"}`}>
                    {formMessage.text}
                  </div>
                )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[11px] font-bold text-muted-foreground mb-1 uppercase tracking-wider">Mã SKU <span className="text-destructive">*</span></label>
                    <input required value={formData.sku} onChange={(e) => setFormData({ ...formData, sku: e.target.value })} className="w-full px-3.5 py-2.5 bg-secondary/20 border border-border rounded-xl text-xs font-semibold focus:ring-2 focus:ring-primary/20 outline-none transition-all" placeholder="VD: LG-001" />
                  </div>
                  <div>
                    <label className="block text-[11px] font-bold text-muted-foreground mb-1 uppercase tracking-wider">Tên phụ tùng <span className="text-destructive">*</span></label>
                    <input required value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full px-3.5 py-2.5 bg-secondary/20 border border-border rounded-xl text-xs font-semibold focus:ring-2 focus:ring-primary/20 outline-none transition-all" placeholder="VD: Lọc gió điều hòa" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[11px] font-bold text-muted-foreground mb-1 uppercase tracking-wider">Nhóm hàng <span className="text-destructive">*</span></label>
                    <input required value={formData.category} onChange={(e) => setFormData({ ...formData, category: e.target.value })} className="w-full px-3.5 py-2.5 bg-secondary/20 border border-border rounded-xl text-xs font-semibold focus:ring-2 focus:ring-primary/20 outline-none transition-all" placeholder="VD: Lọc" />
                  </div>
                  <div>
                    <label className="block text-[11px] font-bold text-muted-foreground mb-1 uppercase tracking-wider">Đơn vị tính <span className="text-destructive">*</span></label>
                    <input required value={formData.unit} onChange={(e) => setFormData({ ...formData, unit: e.target.value })} className="w-full px-3.5 py-2.5 bg-secondary/20 border border-border rounded-xl text-xs font-semibold focus:ring-2 focus:ring-primary/20 outline-none transition-all" placeholder="VD: Cái" />
                  </div>
                </div>

                {/* Parent Product */}
                <div>
                  <label className="block text-[11px] font-bold text-muted-foreground mb-1 uppercase tracking-wider">Mã sản phẩm cha (Mã hàng đa tầng)</label>
                  <CustomSelect
                    value={formData.parentId}
                    onChange={(val) => setFormData({ ...formData, parentId: val })}
                    options={[
                      { value: "", label: "-- Không có (Sản phẩm gốc) --" },
                      ...products
                        .filter((p: any) => !p.parentId && p.id !== editingId)
                        .map((p: any) => ({
                          value: p.id.toString(),
                          label: `${p.sku} - ${p.name}`,
                        })),
                ...
                <div className="pt-2 border-t border-border/40">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[11px] font-bold text-muted-foreground mb-1 uppercase tracking-wider">Min an toàn</label>
                      <NumericInput
                        required
                        value={formData.stockMin}
                        onChange={(val: any) => setFormData({ ...formData, stockMin: val })}
                        className="w-full px-3.5 py-2.5 bg-secondary/20 border border-border rounded-xl text-xs font-semibold focus:ring-2 focus:ring-primary/20 outline-none transition-all"
                      />
                    </div>
                    <div>
                      <label className="block text-[11px] font-bold text-muted-foreground mb-1 uppercase tracking-wider">Max an toàn</label>
                      <NumericInput
                        required
                ...
                <div className="pt-2 border-t border-border/40">
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                      <label className="block text-[11px] font-bold text-muted-foreground mb-1 uppercase tracking-wider">Giá bán lẻ (đ)</label>
                      <NumericInput
                        required
                        value={formData.retailPrice}
                        onChange={(val: any) => setFormData({ ...formData, retailPrice: val })}
                        className="w-full px-3.5 py-2.5 bg-secondary/20 border border-border rounded-xl text-xs font-bold text-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all"
                      />
                    </div>
                    <div>
                      <label className="block text-[11px] font-bold text-muted-foreground mb-1 uppercase tracking-wider">Giá bán sỉ (đ)</label>
                      <NumericInput
                        required
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-muted-foreground mb-1 uppercase tracking-wider">Giá bảo hiểm (đ)</label>
                      <NumericInput
                        required
                ...
                <div className="flex items-center justify-end gap-3 pt-3 border-t border-border/80">
                  ...
                  <button type="submit" disabled={submitting} className="gradient-primary text-white px-6 py-2.5 rounded-xl text-xs font-bold hover:opacity-90 disabled:opacity-60 flex items-center gap-2 shadow-md shadow-primary/20 transition-all">
                    {submitting && <Loader2 size={14} className="animate-spin" />}
                    {submitting ? "Đang lưu..." : editingId ? "Lưu thay đổi" : "Tạo phụ tùng"}
                  </button>
                </div>
              </form>
            </div>
