...
function POST(req) {
    return __awaiter(this, void 0, void 0, function () {
        var _a, email, password, user, _, safeUser, error_1;
        return __generator(this, function (_b) {
            switch (_b.label) {
                ...
                case 2:
                    user = _b.sent();
                    if (!user)
                        return [2 /*return*/, server_1.NextResponse.json({ error: "Email không tồn tại" }, { status: 401 })];
                    // In production: use bcrypt.compare(password, user.password)
                    if (user.password !== password)
                        return [2 /*return*/, server_1.NextResponse.json({ error: "Mật khẩu không đúng" }, { status: 401 })];
                    _ = user.password, safeUser = __rest(user, ["password"]);
                    return [2 /*return*/, server_1.NextResponse.json({ user: safeUser })];
                case 3:
                    error_1 = _b.sent();
