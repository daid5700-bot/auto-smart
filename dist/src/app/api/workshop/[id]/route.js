"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.PATCH = PATCH;
exports.DELETE = DELETE;
var server_1 = require("next/server");
var prisma_1 = require("@/lib/prisma");
// PATCH /api/workshop/[id] — update Repair Order
function PATCH(req_1, _a) {
    return __awaiter(this, arguments, void 0, function (req, _b) {
        var id, body, currentRo, data, ro, tech, commission, points, error_1;
        var _c, _d;
        var params = _b.params;
        return __generator(this, function (_e) {
            switch (_e.label) {
                case 0:
                    _e.trys.push([0, 11, , 12]);
                    id = parseInt(params.id);
                    return [4 /*yield*/, req.json()];
                case 1:
                    body = _e.sent();
                    return [4 /*yield*/, prisma_1.prisma.repairOrder.findUnique({ where: { id: id } })];
                case 2:
                    currentRo = _e.sent();
                    if (!currentRo)
                        return [2 /*return*/, server_1.NextResponse.json({ error: "Lệnh sửa chữa không tồn tại" }, { status: 404 })];
                    data = {
                        plateNumber: body.plateNumber,
                        vehicleModel: body.vehicleModel,
                        kmIn: body.kmIn,
                        symptoms: body.symptoms,
                        status: body.status,
                        technicianId: body.technicianId,
                        laborCost: body.laborCost,
                        partsCost: body.partsCost,
                        totalAmount: ((_c = body.laborCost) !== null && _c !== void 0 ? _c : Number(currentRo.laborCost)) + ((_d = body.partsCost) !== null && _d !== void 0 ? _d : Number(currentRo.partsCost)),
                        photos: body.photos,
                    };
                    if (body.status === "DONE" && currentRo.status !== "DONE") {
                        data.completedAt = new Date();
                    }
                    return [4 /*yield*/, prisma_1.prisma.repairOrder.update({
                            where: { id: id },
                            data: data,
                            include: { customer: true, technician: true },
                        })];
                case 3:
                    ro = _e.sent();
                    if (!(body.status === "DONE" && ro.technicianId && currentRo.status !== "DONE")) return [3 /*break*/, 10];
                    return [4 /*yield*/, prisma_1.prisma.technician.update({ where: { id: ro.technicianId }, data: { status: "IDLE" } })];
                case 4:
                    _e.sent();
                    return [4 /*yield*/, prisma_1.prisma.technician.findUnique({ where: { id: ro.technicianId } })];
                case 5:
                    tech = _e.sent();
                    if (!tech) return [3 /*break*/, 7];
                    commission = Number(ro.totalAmount) * tech.commissionRate / 100;
                    return [4 /*yield*/, prisma_1.prisma.techPerformance.create({
                            data: { technicianId: tech.id, repairOrderId: ro.id, commissionAmount: commission },
                        })];
                case 6:
                    _e.sent();
                    _e.label = 7;
                case 7:
                    points = Math.floor(Number(ro.totalAmount) / 1000);
                    return [4 /*yield*/, prisma_1.prisma.customer.update({
                            where: { id: ro.customerId },
                            data: {
                                loyaltyPoints: { increment: points },
                                totalSpent: { increment: ro.totalAmount },
                                lastVisit: new Date(),
                            },
                        })];
                case 8:
                    _e.sent();
                    return [4 /*yield*/, prisma_1.prisma.znsLog.create({
                            data: {
                                customerId: ro.customerId,
                                phone: ro.customer.phone,
                                messageType: "THANK_YOU",
                                content: "C\u1EA3m \u01A1n kh\u00E1ch h\u00E0ng ".concat(ro.customer.name, " \u0111\u00E3 s\u1EEDa ch\u1EEFa xe ").concat(ro.vehicleModel, ". Qu\u00FD kh\u00E1ch t\u00EDch \u0111\u01B0\u1EE3c +").concat(points, " \u0111i\u1EC3m!"),
                                status: "SENT",
                            },
                        })];
                case 9:
                    _e.sent();
                    _e.label = 10;
                case 10: return [2 /*return*/, server_1.NextResponse.json(ro)];
                case 11:
                    error_1 = _e.sent();
                    return [2 /*return*/, server_1.NextResponse.json({ error: error_1.message }, { status: 400 })];
                case 12: return [2 /*return*/];
            }
        });
    });
}
// DELETE /api/workshop/[id] — delete Repair Order
function DELETE(req_1, _a) {
    return __awaiter(this, arguments, void 0, function (req, _b) {
        var id, error_2;
        var params = _b.params;
        return __generator(this, function (_c) {
            switch (_c.label) {
                case 0:
                    _c.trys.push([0, 2, , 3]);
                    id = parseInt(params.id);
                    return [4 /*yield*/, prisma_1.prisma.repairOrder.delete({ where: { id: id } })];
                case 1:
                    _c.sent();
                    return [2 /*return*/, server_1.NextResponse.json({ success: true, message: "Xóa lệnh sửa chữa thành công" })];
                case 2:
                    error_2 = _c.sent();
                    return [2 /*return*/, server_1.NextResponse.json({ error: error_2.message }, { status: 400 })];
                case 3: return [2 /*return*/];
            }
        });
    });
}
